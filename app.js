...
var showInspiration = function(inspiration) {
	...
	var inspirationElem = result.find('.reputation-text');
	//inspirationElem.attr('href', question.link);
	inspirationElem.text(inspiration.user.reputation);

	var name = result.find('.Name-text');
	//var date = new Date(1000*question.creation_date);
	name.text(inspiration.user.display_name);

	var score = result.find('.Score');
	score.text(inspiration.score);

	var postcnt = result.find('.post-count');
	postcnt.text(inspiration.post_count);
